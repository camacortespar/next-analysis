...
PROCESS_TYPE = 'hpr_background'                            # Options: 'lpr', 'hpr'
...
# Summary file
SUMMARY_FILENAME = 'summary_' + PROCESS_TYPE + '_' + DATE + '_processed.csv'     # Choose your name
...
def parse_arguments():
    ...
    parser.add_argument("isotope",
                        type=str,
                        help="The isotope to process (e.g., 'Bi214', 'Co60', 'K40', 'Tl208').")
...
def process_mc_file(h5_path, cut_names=CUT_NAMES):
    """
    ...
    try:
        # ----- Load Dataframes ----- #
        df_nexus = pd.read_hdf(h5_path, key=MC_CONFIG_KEY)
        df_true  = pd.read_hdf(h5_path, key=TRUE_INFO_KEY)
        df_doro  = pd.read_hdf(h5_path, key=DORO_KEY).loc[:, DORO_COLUMNS]
        df_soph  = pd.read_hdf(h5_path, key=SOPH_KEY).loc[:, SOPH_COLUMNS]
        ...
        print(f"   Failed to process file {h5_path}. Error: {e}", file=sys.stderr)
...
def main():
    ...
    if PROCESS_TYPE in ['lpr_background', 'hpr_background']:

        if PROCESS_TYPE == 'lpr_background': MC_DIR += 'Radiogenics/LPR/IC_v2.3.1/NEXUS_v7_09_00/'
        if PROCESS_TYPE == 'hpr_background': MC_DIR += 'Radiogenics/HPR/IC_v2.3.1/NEXUS_v7_10_01/'

        ISO_DIR = MC_DIR + ISOTOPE + '/'
    ...
    results = Parallel(n_jobs=n_cores)(delayed(process_mc_file)(h5_path) for h5_path in MC_PATHS)
    ...
    output_filename = 'processed_mc_' + PROCESS_TYPE + '_' + ISOTOPE + '_' + DATE + '.h5'
