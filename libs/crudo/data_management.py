...
def summarize_dorothea_info(df_doro: pd.DataFrame, event_level_cols: List[str]) -> pd.DataFrame:
    """
    Aggregates Dorothea data to the event level in a controlled, scalable way.
    ...
    peaks_info_df = df_doro[['event'] + S1_PULSE_COLS + S2_PULSE_COLS].copy()
...
def summarize_hits_to_event_level(df_hits: pd.DataFrame, energy_column='E_hit_mev', size_variable='n_hits') -> pd.DataFrame:
    ...
    total_energy = soph_info_df['E_evt'].replace(0, 1)
    ...
    # --- Clear Output ---
    final_columns = [
                        'E_evt', 'n_cluster', size_variable, 'X_bary', 'Y_bary', 'Z_bary', 
...
def aggregate_to_event_peak_level(df_doro: pd.DataFrame, df_soph: pd.DataFrame, event_level_cols: List[str], energy_column='E_hit_mev') -> pd.DataFrame:
    ...
    doro_info_df = summarize_dorothea_info(df_doro, event_level_cols)       # I want all the peaks info
